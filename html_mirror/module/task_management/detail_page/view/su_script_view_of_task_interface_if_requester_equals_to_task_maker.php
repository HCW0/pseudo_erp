...
<script language="javascript">
				...
				//window.resizeBy(500,500); // 지정한 크기만큼 더하거나 빼져서 변한다.
					...
					function hrefClick_of_delete_task(local){
						...
						window.location.href='../delete/su_script_task_delete_process.php?TID=' + local;
...
function generate_reserve(){
						...
						function reservedFunc(val){
								...
								if(val==1){
									...
									document.getElementById('ajax_state_header').value = 10 ;
														...
														$.ajax({
															url:'../write/ajax_script/realtime_dstate_changer_without_refresh.php',
															dataType:'json',
		...
		<script> 
					...
					var popUrl = "/su_script_gate_view_of_task.php";	//팝업창에 출력될 페이지 URL
	...
	<td  colspan="1">우 선 도</td>
				...
				<div style="padding:10px 0 0 0;">
					...
					<?php echo"<input style='float:right;' type='button' value='승인' onclick='confirm_message(".$target_tid.",1);'/>"; ?>
					<?php echo"<input style='float:right;' type='button' value='반려' onclick='confirm_message(".$target_tid.",2);'/>"; ?>
					</div>
