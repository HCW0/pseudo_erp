...
<?php
	...
	$_SESSION['now_page_coord'] = 1; // 슬라이드 메뉴의 현재 위치 표시에 필요한 전역 변수 값을 변경하는 부분.
...
<?php include('./modal_fab.php');?>
	...
	<script>
		// drag-bar 전용 함수 초기화
						...
						function hrefClick_of_detail_task(level, sub_level, tid) {
							// You can't define php variables in java script as $course etc.
...
<style>
  ...
  #wrapper{
	...
	<div id="wrapper" class="dataTable" style="width:100%"; "height:300px">

			<?php
				$UI_form_ob->su_function_get_title('업무 현황(부서장)',$_SESSION['my_name'],$_SESSION['my_position'],$_SESSION['my_department'],'su_script_user_personal_interface');
			?>
				...
				<div id="head" style="padding:0px 0px 0px 1167px;">
				...
				<br />
					...
					<table>
						...
						<th colspan=2  width="33%" text-align="center">
		...
		<?php
            ...
            ?>
                <tr>
					...
					<td id='left'>
						<?php 
							$squery = "SELECT * FROM task_document_header_table WHERE ".$row['master_task_level_sub_code']." = task_level_sub_code;";
							$sresult = mysqli_query($conn,$squery);

								if(!$sresult || mysqli_num_rows($sresult)==0){
									echo '보유 항목 없음';
									$modal_generate_flag=false;
								}else{
									while($srow = mysqli_fetch_array($sresult)){
											echo"<a href='#' onclick='hrefClick_of_detail_task(".$row['master_task_level_code'].','.$row['master_task_level_sub_code'].','.$srow['TID'].");'>".$srow['task_name']."</a>";
											echo "    ".$ob2->su_function_convert_name($conn,"dmaster_state_info_table","master_code",$srow['task_detail_state'],"master_state_detail_name");
											echo '<br />';
									}
								}
					...
					<td>
						<?php
							...
							$fun= "state_activity_2(this.id,".$row['master_task_level_sub_code'].");";
						?>
					</td>

					<td id='rowdata' onclick=<?php if($modal_generate_flag) echo $fun;?>>
						<?php 
							if($modal_generate_flag){
								 echo "<a href='#' />".$ob2->su_function_convert_name($conn,"dmaster_state_info_table","master_code",$row['task_detail_state'],"master_state_detail_name");
							}else{
						     	 echo "<font color='gray' />".$ob2->su_function_convert_name($conn,"dmaster_state_info_table","master_code",$row['task_detail_state'],"master_state_detail_name");								
							}
						?>
					</td>
...
<div class="testDIV">
      <div class="detail_area">
         <table class="detail_table">			
	  			<tr>
				  <td colspan=4>용역 상태 변경</td>
				</tr>
         </table>

		 <div style="float:right;"><input type='button' class='exit' value='확인' /></div>
		 <div style="float:right;"><input type='button' class='approb' value='진행' /></div>
