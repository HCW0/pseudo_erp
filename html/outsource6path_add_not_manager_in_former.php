...
// 수정/등록 구분자
    ...
    if ($end_user == -1) {
        $msg_ob->su_function_call_message_callback($conn, 914);
        echo "<script> history.go(-1); </script>";
    }

    $duplicate_check = array($end_user);

    for ($cnt = 0; $cnt < 7; $cnt++) {
        if ($_POST['task_select_box'][$cnt] == -1) continue;
        array_push($duplicate_check, $_POST['task_select_box'][$cnt]);
    }
    ...
    foreach($duplicate_check as $var ){
        echo $var;
        echo "<br />";
    }
    ...
    $arr_count = count($duplicate_check);
    $uniq_count = count(array_unique($duplicate_check));

    if ($arr_count != $uniq_count) {
        $msg_ob->su_function_call_message_callback($conn, 915);
        echo "<script> history.go(-1); </script>";
    ...
    if ($end_user > 0) {
        ...
        if ($min != 10000) {
            ...
            $task_detail_table_layer1 = "update task_approbation_path_table set min_sid = 8 where SID = $start AND key_index = $max_key";
            mysqli_query($conn, $task_detail_table_layer1);            
        }
    }
    echo "<script>window.resizeTo(675,575);";
    if($type!=-1){
        echo "window.location.href = './su_script_former_modify_interface.php?notice_id=$type&key=".$max_key."';</script>";
    }else{
        echo "window.location.href = './su_script_former_write_interface.php?key=".$max_key."';</script>";
    }
    ?>
