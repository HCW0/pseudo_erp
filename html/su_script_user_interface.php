...
// class 객체 생성
		...
		$ob2 = new su_class_value_name_convert_with_code();
			$my_name = $ob2->su_function_convert_name($conn,"master_user_info_table","SID",$_SESSION['my_sid_code'],"master_user_info_name");
			$my_position = $ob2->su_function_convert_name($conn,"master_position_info_table","sid_combine_position",$_SESSION['my_position_code'],"master_position_info_name");
			$my_department = $ob2->su_function_convert_name($conn,"master_department_info_table","sid_combine_department",$_SESSION['my_department_code'],"master_department_info_name");
  ...
  th:nth-child(2n+1), td:nth-child(2n+1) {
	...
	<div id="wrapper" style="width:1900px" "height:300px">
			<p align="center"><img src="./src/su_rsc_sulogo_a.png" width="200" height="100" title="선운로고"/></p>
				...
				<span align="left"><?php echo date("Y-m-d H:i:s"); echo "<br />";  ?></span>
				</div>
				<div id="head" style="float:right;">
						
				<form action = 'outsource.php' method='POST' name="table_filter">		
						<span>시작일</span>
						<input type="text" name = "task_select_box[]" id="datepicker1" >
						<span>/ 마감일</span>
						<input type="text"  name = "task_select_box[]" id="datepicker2">
										...
										$query = "SELECT * FROM master_task_level_info_table";
     					        		$result = mysqli_query($conn,$query);  
           										 while( $row=mysqli_fetch_array($result) ){         
														if($row['master_task_level_code']==$_SESSION['current_task_level_code']){
							...
							<select name = "task_select_box[]">	
       							 <?php
										$query = "SELECT * FROM master_task_level_sub_info_table";
     					        		$result = mysqli_query($conn,$query);  
           										 while( $row=mysqli_fetch_array($result) ){    
														if($row['master_task_level_sub_code']==$_SESSION['current_task_level_sub_code']){
							...
							<select  name = "task_select_box[]">	
       							 <?php
										$query = "SELECT * FROM master_department_info_table";
     					        		$result = mysqli_query($conn,$query);  
           										 while( $row=mysqli_fetch_array($result) ){    
															if($row['sid_combine_department']==$_SESSION['current_task_order_section']){
							...
							<select  name = "task_select_box[]">	
       							 <?php
										$query = "SELECT * FROM master_user_info_table";
     					        		$result = mysqli_query($conn,$query);  
           										 while( $row=mysqli_fetch_array($result) ){    
														                 if($row['SID']==$_SESSION['current_task_orderer']){
							...
							<select  name = "task_select_box[]">	
       							 <?php
										$query = "SELECT * FROM master_priority_info_table";
     					        		$result = mysqli_query($conn,$query);  
           										 while( $row=mysqli_fetch_array($result) ){    
														if($row['master_task_priority_info_code']==$_SESSION['current_task_priority']){
							...
							<select  name = "task_select_box[]">	
       							 <?php
										$query = "SELECT * FROM master_state_info_table";
     					        		$result = mysqli_query($conn,$query);  
           										 while( $row=mysqli_fetch_array($result) ){    
														if($row['master_task_state_info_code']==$_SESSION['current_task_state']){
			...
			<?php
				...
				echo "<br />";
				echo '업무 레벨 ';
				echo $_SESSION['current_task_level_code'];
				echo "<br />";
				echo '업무 코드 ';
				echo $_SESSION['current_task_level_sub_code'];
				echo "<br />";
				echo '발주처 코드 ';
				echo $_SESSION['current_task_order_section'];
				echo "<br />";
				echo '발주자 코드 ';
				echo $_SESSION['current_task_orderer'];
				echo "<br />";
				echo '우선도 코드 ';
				echo $_SESSION['current_task_priority'];
				echo "<br />";
				echo '상태 코드 ';
				echo $_SESSION['current_task_state'];
				echo "<br />";
				echo '필터 시작 일자 ';
				echo $_SESSION['current_base_date'];
				echo "<br />";
				echo '필터 제한 일자 ';
		...
		<?php
            ...
            ?>
                <tr>
					...
					?></td>
					<td><input type="submit" name="Release" value="detail"></td>
                </tr>
			...
			<div id="footer" style="padding:600px 0px 0px 1800px;">
						<input type="button" name="버튼" value="업무등록" onclick="window.open('./su_script_table_write_interface.php','win','width=500,height=400,toolbar=0,scrollbars=0,resizable=0')";>
